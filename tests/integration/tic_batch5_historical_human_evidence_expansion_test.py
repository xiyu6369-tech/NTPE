...
from core.translation_intelligence_corpus.failure_corpus_v2 import (
    build_batch5_payloads,
    validate_batch1_through_batch4_anchors,
)
from core.translation_intelligence_corpus.historical_evidence_search import (
...
EXPANSION = load("artifacts/tic_batch5/HISTORICAL_HUMAN_EVIDENCE_EXPANSION.json")
...
def test_batch1_through_batch4_anchor_sha256_values_are_unchanged():
    anchors = validate_batch1_through_batch4_anchors(ROOT)
...
def test_human_evidence_search_is_deterministic():
    first_items, first_report = search_historical_human_evidence(ROOT, CASE_MAP)
    second_items, second_report = search_historical_human_evidence(ROOT, CASE_MAP)
...
def test_stage11_priority_evidence_is_precisely_completed_from_frozen_range():
    ...
    assert item["search_evidence"]["source_disambiguation_artifact"] == "artifacts/te_v72_stage1223/TE_V72_STAGE1223_SOURCE_EXCERPT_FREEZE.json"
...
def test_duplicate_evidence_cannot_create_duplicate_failure_case(monkeypatch):
    original_search = batch5.search_historical_human_evidence

    def duplicate(root, cases):
        items, report = original_search(root, cases)
        aligned = next(item for item in items if item["usable_for_failure_corpus"])
        return items + [copy.deepcopy(aligned)], report

    monkeypatch.setattr(batch5, "search_historical_human_evidence", duplicate)
    with pytest.raises(ValueError, match="duplicate failure evidence"):
        build_batch5_payloads(ROOT)
...
def test_canonical_rebuild_matches_all_batch5_artifacts():
    rebuilt = build_batch5_payloads(ROOT)
