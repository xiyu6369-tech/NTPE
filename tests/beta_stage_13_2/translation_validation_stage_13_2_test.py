"""Translation validation guard for Stage-13.2."""
import os
import sys
...
def test_translation_validation_stage_13_2():
    ...
    assert manifest["dashboard_stage"] == "13.2"
    page = app.render("/").to_dict()
    assert page["state"]["rest_api_available"] is True
    assert any(component["type"] == "dashboard" for component in page["components"])
