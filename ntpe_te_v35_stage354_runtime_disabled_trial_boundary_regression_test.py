...
MANIFEST_PATH = ROOT / "manifests" / "te_v35_runtime_disabled_trial_boundary_manifest.json"
LAUNCHER_PATH = ROOT / "launcher_translate.py"
SECRET_TEXT = "disabled trial boundary secret text"


def test_disabled_trial_boundary_imports_and_manifest() -> None:
    ...
    assert manifest["stage"] == "3.5.4"
    assert manifest["layer"] == "runtime_disabled_trial_boundary"
    assert "RuntimeDisabledTrialMockBridge" in manifest["components"]
...
def test_disabled_trial_boundary_blocked_and_mock_paths() -> None:
    bridge = RuntimeDisabledTrialMockBridge()
    request = {
        "request_type": "disabled_trial",
        "runtime_id": "boundary-354",
    ...
    assert completed["trial_guard_result"]["safety_boundaries"]["provider_runtime"] == "forbidden"
...
def test_disabled_trial_boundary_no_runtime_provider_http_api_key_or_launcher_side_effects() -> None:
    ...
    try:
        bridge = RuntimeDisabledTrialMockBridge()
        result = bridge.run(
            request={"request_type": "disabled_trial", "runtime_id": "boundary-354", "text": SECRET_TEXT},
...
def main() -> int:
    test_disabled_trial_boundary_imports_and_manifest()
    test_disabled_trial_boundary_blocked_and_mock_paths()
    test_disabled_trial_boundary_no_runtime_provider_http_api_key_or_launcher_side_effects()
    print("NTPE TE-v3.5 Stage-3.5.4 Runtime Disabled Trial Boundary Regression PASS")
    return 0
