...
def write_authorized_harness_report(
    result: AuthorizedProviderHarnessResult, path: str | Path,
) -> Path:
    payload = result.to_dict()
    payload["content_redacted"] = True
    assert_redacted(payload)
    payload["artifact_sha256"] = {
        "algorithm": "sha256", "payload_sha256": report_sha256(payload),
    }
    target = Path(path)
...
def verify_authorized_harness_report(path: str | Path) -> dict[str, object]:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    integrity = payload.pop("artifact_sha256", {})
    if integrity.get("payload_sha256") != report_sha256(payload):
        raise ValueError("authorized Provider harness artifact integrity failure")
    assert_redacted(payload)
    return payload
