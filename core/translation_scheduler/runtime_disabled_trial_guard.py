...
class RuntimeDisabledTrialGuard:
    """Validate disabled trial requests before any runtime hook path."""
    ...
    def __init__(self, contract_builder: RuntimeDisabledTrialContract | None = None) -> None:
    ...
    def validate_guard_result(self, result: Mapping[str, Any] | None) -> dict[str, Any]:
        ...
        if data.get("stage") != self.stage:
            errors.append("stage must be 3.5.2")
        if data.get("trial_status") not in {"allowed", "blocked"}:
            errors.append("trial_status must be allowed or blocked")
        if data.get("allowed") is True and data.get("trial_status") != "allowed":
            errors.append("allowed result must have allowed trial_status")
        if data.get("blocked") is True and data.get("trial_status") != "blocked":
            errors.append("blocked result must have blocked trial_status")
...
__all__ = ["RuntimeDisabledTrialGuard"]
