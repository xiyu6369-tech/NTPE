...
from .integrity import report_sha256
from .result import ControlledSessionResult


def write_session_report(result: ControlledSessionResult, path: str | Path) -> Path:
...
def verify_session_report(path: str | Path) -> dict[str, object]:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    integrity = payload.pop("artifact_sha256", {})
    if integrity.get("payload_sha256") != report_sha256(payload):
        raise ValueError("controlled session artifact integrity failure")
    assert_redacted(payload)
    return payload
