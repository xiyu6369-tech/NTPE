#!/usr/bin/env python3
"""
P0-FINAL-15-S1: GPT-OSS 120B Context & Quality Recovery Investigation (Optimized)

Focused investigation with reasonable timeouts.
"""
...
def load_fixtures() -> Dict[str, str]:
    return {
        "narrative": "정태의는 난감해하고 있었다. 그러나 실상 그것은 그가 난감해할 일은 아니었다. 먼 타국에 떼어놓고 온 괴물 같은 남자는 어쨌든 이지가 제대로 돌아가고 있는, 나름대로 이성적인 인간이었고, 그는 이 상황이 결코 정태의가 의도해서 벌어진 상황이 아니란 걸 이해해줄 것이다. 그러나, '몇 달 전부터 벼르고 별러서 천신만고 끝에 겨우 일주일 휴가를 뺐는데, 그놈이 일 때문에 예정보다 늦게 돌아온다고 해서 내 휴가를 깎아먹을 수는 없다'라고 분연히 주장하며, 이웃나라까지 일하러 간 동생을 내팽개치고 정태의만 홀랑 데리고 머나먼 남국의 섬으로 휴가를 와버린 카일이 무사할 수 있을지가 사뭇 걱정이 되었다. (카일도 평소의 이성적인 그였더라면 이런 짓을 막 벌이진 않았을 테지만, 일하러 떠나기 직전에 카일이 아끼던 책을 불살라버린 동생의 작태에 사흘을 앓아누울 정도로 광분했던 탓이 다분했다.) 어찌 되었든, 직통이라곤 없이 경비행기로 따로 들어와야 하는 이 여유로운 남국의 섬에 있는 호텔의 로비 소파에 앉아, 지금이야 좋지만 베를린으로 돌아간 뒤에 어떤 사단이 날지 그저 걱정스럽기만 한 정태의는 한숨을 쉬고 있었다. '……하지만 지금 미리 걱정해본들 뭐 이미 벌어진 일은 어쩔 도리 없고……. 모처럼 쉬러 왔는데 한숨만 쉬며 일주일을 지내면 손해지.' 정태의는 결국 합리적인 결론을 내리고 고개를 끄덕였다. 짙은 청옥빛 바다로 둘러싸인 이 섬은 대단히 아름답고 한가로웠다. 오는 길에 카일에게 들은 바로는, 이곳의 호텔은 몇몇 재벌이 소유한 별장처럼 쓰이다시피 해 일반 관광객은 들어오지 못하기 때문에, 남의 눈에 띌 걱정 없이 한가롭고 넉넉한 휴가를 보내기에는 안성맞춤이라고 했다. 과연 그의 말마따나, 어제 이곳에 들어온 이후 정태의가 구경한 사람이라곤 이곳의 관리인과 직원 외에는 열 손가락으로 꼽고도 넉넉하게 남을 정도였다. '인적 드문 바닷가 산책도 좋겠지.' 정태의는 어차, 하고 자리에서 일어섰다. 카일은 프라이�트풀 옆의 벤치에서 정신없이 잠들어 있을 터였다. (일주일의 휴가를 위해 그가 이곳에 오기 직전까지 밤을 새며 퀭한 얼굴로 일했다는 걸 정태의는 알고 있었다. 그래서, 그가 막무가내로 여기에 오겠다고 하는 주장을 차마 거를 수 없었다.) 라군에서 바닷가는, 당연하다면 당연하지만, 엎어지면 코 닿을 거리였다. 근처만 서성이려면 바다 위로 뻗은 나무다리 위로 산책을 할 수도 있지만, 정태의는 천천히 섬을 한 바퀴 돌아보자고 생각하고 백사장 쪽을 선택했다. 대충 걸어서 두어 시간이면 한 바퀴 다 돌 수 있을 정도로 작은 섬이라고 들었다. 무릅까지 오는 반바지 위에 넉넉한 셔츠 하나만 대충 걸치고, 그가 막 로비에서 나서려던 때였다. 새로운 손님이 왔는지, 바깥에서 약간 떠들썩하게―라고 해도 이곳이 워낙 조용해서, 단순한 말소리조차 그렇게 들릴 뿐이었지만―두세 명이 들어서고 있었다. 정태的는 저도 모르게 걸음을 멈추었다. 귀에 익은 독일어가 들렸다.",
...
def run_context_boundary(api_key: str, endpoint: str, model: str, fixtures: dict, glossary_prompt: str) -> List[S1Result]:
    print("\n=== Context Boundary Sweep ===")
    narrative = fixtures["narrative"]
    results = []
    boundaries = [0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]
    ...
    for pct in boundaries:
        ...
        status, elapsed, content, finish_reason, req_id, nvcf_id, error = run_single(
        ...
        r = S1Result(
            test_id=test_id, fixture="narrative", mode="glossary", context_pct=pct,
...
def run_chunking(api_key: str, endpoint: str, model: str, fixtures: dict, glossary_prompt: str) -> List[S1Result]:
    print("\n=== Chunking Investigation ===")
    narrative = fixtures["narrative"]
    results = []
    strategies = [("single", 1), ("small", 4), ("medium", 3), ("large", 2)]
    ...
    for name, count in strategies:
        ...
        r = S1Result(
            test_id=test_id, fixture="narrative", mode="glossary_chunked", context_pct=1.0,
...
def run_quality_matrix(api_key: str, endpoint: str, model: str, fixtures: dict, base_prompt: str, glossary_prompt: str) -> List[S1Result]:
    print("\n=== Quality Matrix (Base vs Glossary) ===")
    results = []
    ...
    for fixture_name, source in fixtures.items():
        for mode, prompt in [("base", base_prompt), ("glossary", glossary_prompt)]:
            ...
            r = S1Result(
                test_id=test_id, fixture=fixture_name, mode=mode, context_pct=1.0,
                http_status=status, success=status==200, elapsed_s=elapsed,
...
def run_reduced_context(api_key: str, endpoint: str, model: str, fixtures: dict, glossary_prompt: str) -> List[S1Result]:
    ...
    for pct in pcts:
        for fixture_name, source in fixtures.items():
            test_id = f"REDUCED_{fixture_name}_{int(pct*100)}"
            user_prompt = source[:int(len(source)*pct)]
            ...
            r = S1Result(
                test_id=test_id, fixture=fixture_name, mode="glossary_reduced", context_pct=pct,
                http_status=status, success=status==200, elapsed_s=elapsed,
                input_chars=len(user_prompt), output_chars=len(content),
                truncation=trunc, preservation=preserv, quality=quality, quality_pass=qpass,
                finish_reason=fr or "unknown", error=error
...
def run_output_budget(api_key: str, endpoint: str, model: str, fixtures: dict, glossary_prompt: str) -> List[S1Result]:
    ...
    for fixture_name, source in fixtures.items():
        ...
        r = S1Result(
            test_id=test_id, fixture=fixture_name, mode="glossary_budget", context_pct=1.0,
            http_status=status, success=status==200, elapsed_s=elapsed,
            input_chars=len(source), output_chars=len(content),
            truncation=trunc, preservation=preserv, quality=quality, quality_pass=qpass,
            finish_reason=fr or "unknown", error=error
...
def save_results(baseline: dict, all_results: List[S1Result]):
...
""")
        ...
        conditional = any(r.success and not r.truncation and r.quality_pass and r.mode in ["glossary", "glossary_reduced"] for r in all_results)
...
def main():
    ...
    all_results.extend(run_context_boundary(api_key, endpoint, model, fixtures, glossary_prompt))
    all_results.extend(run_chunking(api_key, endpoint, model, fixtures, glossary_prompt))
    all_results.extend(run_quality_matrix(api_key, endpoint, model, fixtures, base_prompt, glossary_prompt))
    all_results.extend(run_reduced_context(api_key, endpoint, model, fixtures, glossary_prompt))
    all_results.extend(run_output_budget(api_key, endpoint, model, fixtures, glossary_prompt))
    
    # Save
    save_results(baseline, all_results)
