#!/usr/bin/env python3
"""
Phase 3A.2 — DeepSeek V4 Candidate Availability Probe

Evaluates DeepSeek V4 models against NTPE Translation Contract on NVIDIA provider.
Does NOT modify production behavior.

Layers:
0. Baseline Integrity
1. Catalog / Model Identity
2. NVIDIA Endpoint Availability
...
@dataclass
class DeepSeekCandidate:
    """DeepSeek V4 candidate model configuration."""
    model_id: str
    name: str
    variant: str
    catalog_status: str = "UNKNOWN"
    model_family: str = "DeepSeek V4"
    model_type: str = "General Purpose LLM"
    context_info: str = ""
...
class EvaluationReport:
    """Complete Phase 3A.2 evaluation report"""
    # Baseline
    head_commit: str
    ...
    # Candidates
    candidates: list[DeepSeekCandidate]
...
def check_baseline_integrity() -> tuple[bool, str, str]:
    """Check that Phase 3A.2 hasn't modified production code."""
    import subprocess
...
def get_deepseek_candidates() -> list[DeepSeekCandidate]:
    """Define DeepSeek V4 candidate models for Phase 3A.2 evaluation."""
    ...
    return [
        DeepSeekCandidate(
            ...
            context_info="128K+ context, reasoning capable",
            availability_timestamp=timestamp,
            source_evidence="NVIDIA catalog (DeepSeek hosted); v4-pro dated variant",
            notes="DS-V4-PRO - Primary candidate; dated variant for stability",
        ),
        DeepSeekCandidate(
            ...
            catalog_status="UNKNOWN",
            model_family="DeepSeek V4",
            model_type="Lightweight LLM",
            context_info="128K+ context, optimized for speed",
            availability_timestamp=timestamp,
            source_evidence="NVIDIA catalog (DeepSeek hosted); v4-flash dated variant",
            notes="DS-V4-FLASH - Primary candidate; flash variant for throughput",
        ),
        DeepSeekCandidate(
            ...
            context_info="128K+ context, reasoning capable",
            availability_timestamp=timestamp,
            source_evidence="NVIDIA catalog (DeepSeek hosted); base v4-pro variant",
            notes="DS-V4-PRO-BASE - Secondary candidate; only if primary unavailable",
        ),
        DeepSeekCandidate(
            ...
            context_info="128K+ context, optimized for speed",
            availability_timestamp=timestamp,
            source_evidence="NVIDIA catalog (DeepSeek hosted); base v4-flash variant",
            notes="DS-V4-FLASH-BASE - Secondary candidate; only if primary unavailable",
        ),
    ]


def run_layer1_catalog(candidate: DeepSeekCandidate) -> Layer1Result:
    ...
    if candidate.model_id in known_models:
        ...
        return Layer1Result(
            ...
            success=False,
            provider="NVIDIA",
            catalog_status="MODEL_NOT_FOUND",
            model_family="",
            model_type="",
            context_info="",
...
def run_layer2_endpoint(candidate: DeepSeekCandidate, api_key: str, endpoint: str) -> Layer2Result:
    """Layer 2: NVIDIA Endpoint Availability - Minimal request to check availability."""
...
def classify_http_error(status: int, body: str) -> str:
    ...
    if status == 200:
        ...
        if "not found" in body_lower and ("function" in body_lower or "model" in body_lower):
...
def run_layer3_minimal_generation(candidate: DeepSeekCandidate, api_key: str, endpoint: str) -> Layer3Result:
...
def load_ntpe_fixtures() -> dict[str, dict]:
    """Load NTPE translation test fixtures (reused from Phase 3A)."""
    fixtures = {}
    
    # Fixture 1: Narrative from Golden Set
    ...
    # Fixture 2: Dialogue-heavy excerpt
    fixtures["dialogue"] = {
        "name": "dialogue",
        "type": "dialogue",
        "source": (
    ...
    # Fixture 3: Context-dependent (continuity)
    fixtures["context"] = {
        "name": "context",
        "type": "context",
        "source": (
...
def run_layer4_contract_probe(candidate: DeepSeekCandidate, fixture: dict, api_key: str, endpoint: str) -> Layer4Result:
    """Layer 4: NTPE Contract Probe - Test against NTPE Translation Contract."""
    
    timestamp_utc = datetime.datetime.utcnow().isoformat() + "Z"
    
    # NTPE-style translation prompt (from baseline - MUST NOT MODIFY)
    system_prompt = (
...
def check_ntpe_contract(translation: str, fixture: dict) -> dict:
    ...
    semantic_complete = len(translation) >= len(source) * 0.3
    checks["semantic_complete"] = semantic_complete
    
    # 4. Dialogue distinction
...
def classify_candidate(verdict: CandidateVerdict) -> tuple[str, Optional[str], list[str]]:
    ...
    # Layer 0 - Baseline
    if verdict.layer0 and not verdict.layer0.match:
        return "BASELINE_MISMATCH", f"HEAD {verdict.layer0.head_commit} != expected {verdict.layer0.expected_commit}", []
    
    # Layer 1
    if verdict.layer1 and not verdict.layer1.success:
        if verdict.layer1.catalog_status == "MODEL_NOT_FOUND":
    ...
    if verdict.layer2:
        ...
        elif verdict.layer2.error_category == "GONE":
            return "MODEL_EOL_OR_RETIRED", f"Model/endpoint retired (HTTP 410): {verdict.layer2.error}", []
        elif verdict.layer2.error_category == "MODEL_NOT_FOUND":
            return "MODEL_NOT_FOUND", f"Model not found (HTTP 404): {verdict.layer2.error}", []
        elif verdict.layer2.error_category in ["UNAUTHORIZED", "FORBIDDEN"]:
            return "PROVIDER_UNAVAILABLE", f"Provider auth error: {verdict.layer2.error_category}", []
        elif verdict.layer2.error_category == "NOT_FOUND":
            return "PROVIDER_UNAVAILABLE", f"Endpoint/Model not found for account: {verdict.layer2.error}", []
    ...
    # Layer 4
    layer4_success = [r for r in verdict.layer4_results if r.success]
    if not layer4_success:
        return "AVAILABLE_INCOMPATIBLE", "All NTPE contract probes failed", []
    
    contract_rates = [r.contract_pass_rate for r in layer4_success]
    avg_contract_rate = sum(contract_rates) / len(contract_rates) if contract_rates else 0
    
    recommendations = []
    
    if avg_contract_rate >= 0.8:
        return "AVAILABLE_COMPATIBLE", None, ["Recommended for Phase 3B"]
    elif avg_contract_rate >= 0.6:
        recommendations.append("Partial compatibility - review specific contract failures")
        return "AVAILABLE_PARTIAL", f"Contract pass rate: {avg_contract_rate:.0%}", recommendations
    else:
        recommendations.append("Fundamental NTPE contract incompatibility")
        return "AVAILABLE_INCOMPATIBLE", f"Contract pass rate: {avg_contract_rate:.0%}", recommendations


def main():
    """Main entry point for Phase 3A.2 evaluation."""
    print("=" * 80)
    print("PHASE 3A.2 — DeepSeek V4 Candidate Availability Probe")
    print("=" * 80)
    
    # Layer 0: Baseline Integrity
    print("\n[LAYER 0] Baseline Integrity Check...")
    baseline = get_git_baseline()
    expected_commit = "af5cbc0"
    head_match = baseline["head_commit"].startswith(expected_commit)
    ...
    layer0 = Layer0Result(
        ...
        contamination=not integrity_pass,
    )
    
    print(f"  HEAD: {baseline['head_commit'][:12]}")
    print(f"  Expected: {expected_commit}")
    print(f"  Match: {head_match}")
    print(f"  Baseline Integrity: {'PASS' if integrity_pass else 'FAIL'}")
    
    if not head_match:
        print(f"  ERROR: BASELINE_MISMATCH - HEAD is not af5cbc0!")
        return 1
    
    if not integrity_pass:
        print(f"  ERROR: BASELINE_CONTAMINATION DETECTED!")
        return 1
    ...
    # Get candidates
    candidates = get_deepseek_candidates()
    print(f"\n[CANDIDATES] {len(candidates)} DeepSeek V4 models to evaluate")
    
    # Load fixtures
    fixtures = load_ntpe_fixtures()
    print(f"[FIXTURES] {len(fixtures)} translation fixtures loaded (reused from Phase 3A)")
    ...
    for candidate in candidates:
        ...
        # If Layer 2 fails with GONE, skip remaining
        if layer2.error_category == "GONE":
            verdict = CandidateVerdict(
                model_id=candidate.model_id,
                name=candidate.name,
                layer0=layer0,
                ...
                layer3=None,
                layer4_results=[],
                final_verdict="MODEL_EOL_OR_RETIRED",
                failure_reason=f"Model/endpoint retired at Layer 2: {layer2.error}",
            )
            verdicts.append(verdict)
            continue
        ...
        # If Layer 2 fails with MODEL_NOT_FOUND (404 with model-specific message)
        if layer2.error_category == "MODEL_NOT_FOUND":
            verdict = CandidateVerdict(
                model_id=candidate.model_id,
                name=candidate.name,
                layer0=layer0,
                ...
                layer4_results=[],
                final_verdict="MODEL_NOT_FOUND",
                failure_reason=f"Model not found at Layer 2: {layer2.error}",
            )
            verdicts.append(verdict)
            continue
    ...
    for verdict in verdicts:
        compatibility_matrix[verdict.model_id] = {
            "provider": "NVIDIA",
            "catalog": "PASS" if verdict.layer1 and verdict.layer1.success else "FAIL",
            "endpoint": "PASS" if verdict.layer2 and verdict.layer2.http_status == 200 else f"FAIL ({verdict.layer2.error_category if verdict.layer2 else 'N/A'})",
            "generation": "PASS" if verdict.layer3 and verdict.layer3.success else "FAIL",
            "contract": {
    ...
    model_eol_or_retired = [v.model_id for v in verdicts if v.final_verdict == "MODEL_EOL_OR_RETIRED"]
    ...
    baseline_mismatch = [v.model_id for v in verdicts if v.final_verdict == "BASELINE_MISMATCH"]
    ...
    if baseline_mismatch:
        phase_verdict = "P3A2_BLOCKED"
    elif available_compatible and integrity_pass:
        phase_verdict = "P3A2_PASS"
    ...
    report = EvaluationReport(
        ...
        model_eol_or_retired=model_eol_or_retired,
    ...
    # Save artifacts
    artifacts_dir = Path(__file__).resolve().parents[2] / "artifacts" / "p3a2_deepseek_probe"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    
    report_path = artifacts_dir / "P3A2_DEEPSEEK_V4_COMPATIBILITY_MATRIX.json"
    ...
    gov_path = governance_dir / "P3A2_DEEPSEEK_V4_COMPATIBILITY_PROBE.md"
    ...
    # Final summary
    print("\n" + "=" * 80)
    print("PHASE 3A.2 EVALUATION COMPLETE")
    print("=" * 80)
    ...
    print(f"  MODEL_EOL_OR_RETIRED: {len(model_eol_or_retired)} - {', '.join(model_eol_or_retired) if model_eol_or_retired else 'None'}")
    ...
    return 0 if phase_verdict in ["P3A2_PASS", "P3A2_PARTIAL"] else 1
...
def generate_governance_markdown(report: EvaluationReport) -> str:
    ...
    lines = []
    lines.append("# P3A2_DEEPSEEK_V4_COMPATIBILITY_PROBE — Phase 3A.2 DeepSeek V4 Availability Probe")
    lines.append("")
    lines.append("## Phase Objective")
    lines.append("")
    lines.append("Evaluate DeepSeek V4 candidate models on the **PRE-MINIMAX-RECONSTRUCTED-BASELINE** ")
    lines.append("to confirm which candidates are actually connectable, callable, and produce ")
    lines.append("NTPE Translation Contract-compliant output on the NVIDIA provider.")
    lines.append("")
    lines.append("## Baseline Lock")
    lines.append("")
    lines.append(f"- **HEAD**: `{report.head_commit}`")
    lines.append(f"- **Expected**: `af5cbc0`")
    lines.append(f"- **Match**: {report.baseline_integrity_pass and report.verdicts[0].layer0.match if report.verdicts else False}")
    ...
    lines.append("")
    lines.append("| Candidate | Model ID | Variant | Catalog | Family | Type | Context | Source Evidence | Notes |")
    lines.append("|-----------|----------|---------|---------|--------|------|---------|-----------------|-------|")
    for c in report.candidates:
        lines.append(f"| {c.name} | `{c.model_id}` | {c.variant} | {c.catalog_status} | {c.model_family} | {c.model_type} | {c.context_info} | {c.source_evidence} | {c.notes} |")
    lines.append("")
    ...
    # Layer 0
    if report.verdicts:
        v0 = report.verdicts[0]
        if v0.layer0:
            lines.append("### Layer 0 — Baseline Integrity")
            lines.append("")
            lines.append("| HEAD Commit | Expected | Match | Clean |")
            lines.append("|-------------|----------|-------|-------|")
            lines.append(f"| {v0.layer0.head_commit[:12]} | {v0.layer0.expected_commit} | {v0.layer0.match} | {not v0.layer0.contamination} |")
            lines.append("")
    ...
    for v in report.verdicts:
        if v.layer1:
            lines.append(f"| {v.layer1.model_id} | {v.layer1.provider} | {v.layer1.catalog_status} | {v.layer1.model_family} | {v.layer1.model_type} | {v.layer1.context_info} | {v.layer1.source_evidence} | {'PASS' if v.layer1.success else 'FAIL'} |")
    lines.append("")
    
    # Layer 2
    lines.append("### Layer 2 — NVIDIA Endpoint Availability")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Model | Name | Verdict | Failure Reason | Recommendations |")
    lines.append("|-------|------|---------|----------------|-----------------|")
    for v in report.verdicts:
        reason = v.failure_reason or "—"
        recs = "; ".join(v.recommendations) if v.recommendations else "—"
        lines.append(f"| {v.model_id} | {v.name} | **{v.final_verdict}** | {reason} | {recs} |")
    lines.append("")
    
    # Compatibility Matrix
    lines.append("## Compatibility Matrix")
    lines.append("")
    lines.append("| Candidate | Provider | Model ID | Catalog | Endpoint | Generation | Contract | Verdict |")
    lines.append("|-----------|----------|----------|---------|----------|------------|----------|---------|")
    for model_id, matrix in report.compatibility_matrix.items():
        contract_str = ", ".join([f"{k}:{v}" for k, v in matrix.get("contract", {}).items()]) if matrix.get("contract") else "N/A"
        lines.append(f"| {model_id} | {matrix['provider']} | {model_id} | {matrix['catalog']} | {matrix['endpoint']} | {matrix['generation']} | {contract_str} | **{matrix['verdict']}** |")
    lines.append("")
    ...
    lines.append(f"- **MODEL_EOL_OR_RETIRED**: {len(report.model_eol_or_retired)} — {', '.join(report.model_eol_or_retired) if report.model_eol_or_retired else 'None'}")
    ...
    lines.append("")
    lines.append("## Phase 3A (Previous) Candidates Comparison")
    lines.append("")
    lines.append("| Phase 3A Candidate | Provider | Verdict |")
    lines.append("|--------------------|----------|---------|")
    lines.append("| meta/llama-3.2-90b-vision-instruct | NVIDIA | AVAILABLE_PARTIAL |")
    lines.append("| nvidia/riva-translate-4b-instruct-v2 | NVIDIA | AVAILABLE_PARTIAL |")
    lines.append("| meta/llama-3.3-70b-instruct | NVIDIA | AVAILABLE_INCOMPATIBLE (EOL/410) |")
    lines.append("| minimaxai/minimax-m3 | MiniMax | AVAILABLE_INCOMPATIBLE (429) |")
    lines.append("| nvidia/llama-3.1-nemotron-70b-instruct | NVIDIA | PROVIDER_UNAVAILABLE (404) |")
    lines.append("")
    lines.append("## DeepSeek V4 Specific Observations")
    lines.append("")
    for v in report.verdicts:
        lines.append(f"### {v.name} ({v.model_id})")
        lines.append("")
        if v.layer2:
            lines.append(f"- **Endpoint Status**: HTTP {v.layer2.http_status} ({v.layer2.error_category})")
            lines.append(f"- **Latency**: {v.layer2.elapsed_ms:.0f}ms")
        if v.layer3:
            lines.append(f"- **Generation**: {'Success' if v.layer3.success else 'Failed'}")
            lines.append(f"- **Finish Reason**: {v.layer3.finish_reason or 'N/A'}")
        if v.layer4_results:
            successful = [r for r in v.layer4_results if r.success]
            if successful:
                avg_rate = sum(r.contract_pass_rate for r in successful) / len(successful)
                lines.append(f"- **Avg Contract Pass Rate**: {avg_rate:.0%}")
                for r in successful:
                    lines.append(f"  - {r.fixture_name}: {r.contract_pass_rate:.0%}")
        lines.append("")
    ...
    lines.append("- ✅ No prompt/contract modification")
    ...
    lines.append("- ✅ Evidence saved to artifacts/p3a2_deepseek_probe/ only")
    lines.append("- ✅ Existing Phase 3A candidates NOT re-run")
    lines.append("")
    lines.append("## Phase Boundary")
    lines.append("")
    lines.append("**Phase 3A.2 COMPLETE — STOP**")
    lines.append("")
    lines.append("Do NOT:")
    ...
    lines.append("- Modify tests")
    lines.append("- Commit")
    lines.append("- Push")
    lines.append("- Proceed to production migration")
    lines.append("")
    lines.append("Next step: Human review of **P3A + P3A.2** compatibility evidence to decide Phase 3B candidate set.")
