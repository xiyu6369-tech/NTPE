...
with tempfile.TemporaryDirectory() as tmp:
    ...
    # Disable validation
    import core.epub_translation.runtime.epub_packager as packager_module
    packager_module._validate_epub_archive = lambda path: []
    
    # Disable CSS from extracted resources
    original_css = packager_module._build_css_from_resources
    packager_module._build_css_from_resources = lambda resources: "body { font-family: serif; }"
    
    # Disable contract resource processing
    original_pack = packager_module.pack_epub_resource_aware
    # We can't easily disable the contract resource loop without modifying the function
    # Let's just run with the current code but with CSS fixed
