...
old_code = '''class TestS5BB23InvalidSpineFailure:
    ...
    def test_invalid_spine_validation(self, output_path: Path) -> None:
        ...
        with zipfile.ZipFile(bad_epub, 'w') as z:
            ...
            ET.SubElement(metadata, "dc:identifier", id="bookid").text = "test"
            ...
            ET.SubElement(manifest, "item", id="ch1", href="ch1.xhtml", media_type="application/xhtml+xml")
            # Spine references non-existent item
            spine = ET.SubElement(opf, "spine")
            ET.SubElement(spine, "itemref", idref="nonexistent")
            z.writestr("content.opf", ET.tostring(opf, encoding="unicode"))
