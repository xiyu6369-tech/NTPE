...
# R1-A expected files (from R1-A report)
r1_a_files = {
...
# R1-B expected test files (from R1-B report)
r1_b_test_files = {
...
# R1-C expected tools files (from R1-C report)
r1_c_files = {
...
# R1-C preserved (output dir / historical)
r1_c_preserved = {
...
for f in changed_files:
    if f in r1_a_files:
        ...
        classified[f] = 'R1-B'
    elif f in r1_b_fixtures:
        classified[f] = 'R1-B'
    elif f in r1_c_files:
        classified[f] = 'R1-C'
    elif f in r1_c_preserved:
        classified[f] = 'R1-C-PRESERVED'
...
# List by category
for cat in ['R1-A', 'R1-B', 'R1-C', 'R1-C-PRESERVED', 'R1-D', 'R1-INVENTORY', 'PROTECTED_WORKTREE', 'UNKNOWN']:
    files = [f for f, c in classified.items() if c == cat]
    if files:
        print(f"\n=== {cat} ({len(files)}) ===")
