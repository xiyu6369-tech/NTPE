    ...
    def _on_import_epub(self) -> None:
        ...
        try:
            ...
            if extraction_result.status == "success":
                ...
                # 即使 success 也可能有 warnings，需顯示
                if extraction_result.warnings:
                    QMessageBox.information(
                        self,
                        "警告",
                        "匯入成功，但有以下警告：\n" + "; ".join(extraction_result.warnings),
                    )
                self.epub_imported.emit(book_info)

            elif extraction_result.status == "partial":
                # 部分成功 - 明確提示
                QMessageBox.warning(
                    self,
                    Strings.EPUB_IMPORT_PARTIAL_TITLE,
                    Strings.EPUB_IMPORT_PARTIAL_MSG.format(
                        title=book_info.get("title", Strings.UNKNOWN),
                        source=book_info.get("source", str(source_path)),
                        chapters=book_info.get("chapters", 0),
                        chars=book_info.get("chars", 0),
                ...
                # manual_review_required, blocked 等非成功狀態
